...
async function runAudit() {
    ...
    const patCookie = patVerify.setCookie;
    
    const search = await request('http://localhost:3000/api/public/search?q=');
    const doctor = search.data.results[0];
    
    const book = await request('http://localhost:3000/api/patient/queue/book-token', 'POST', {
        doctorId: doctor.id,
        isEmergency: false,
        location: 'Patna'
    }, patCookie);
    console.log('Book Token Details:', book.status, JSON.stringify(book.data));
}
