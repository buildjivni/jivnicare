      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="flex flex-col md:flex-row items-end justify-between mb-12 gap-6">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50/80 text-[#205E98] text-xs font-bold uppercase tracking-widest mb-4">
          ...
          <Link href="/doctors" className="hidden md:inline-flex shrink-0 items-center gap-2 font-bold text-[#205E98] hover:text-[#184a7a] transition-colors">
        ...
        >
          {SPECIALTIES.map((spec, i) => {
            ...
            const colors = [
              "bg-blue-50 text-[#205E98] border-blue-100/50",
                ...
                <div className={`group flex items-center p-4 md:p-5 rounded-2xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 cursor-pointer bg-white hover:border-transparent ${colors[i % colors.length].split(" ")[2]}`}>
                  ...
                  <div className="ml-4 md:ml-5">
                    ...
                    <p className="text-[10px] md:text-xs font-semibold text-slate-500 flex items-center gap-1">
                      Book Now <ArrowRight className="w-3 h-3 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-[#205E98]" />
                    </p>
                  </div>
                </div>
              </Link>
        ...
        <div className="mt-8 md:hidden flex justify-center">
          <Link href="/doctors" className="inline-flex items-center gap-2 font-bold text-[#205E98] bg-blue-50 px-6 py-3 rounded-full">
            View All Departments <ArrowRight className="w-5 h-5" />
          </Link>
        </div>
      </div>
