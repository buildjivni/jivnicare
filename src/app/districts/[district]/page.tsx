    ...
    <main className="min-h-screen bg-[#f7f9fc] pb-20">
        ...
        <div className="container mx-auto max-w-5xl">
          ...
          <div className="flex flex-wrap gap-3 mt-8">
            <Link href={`/doctors?district=${districtFormatted}`}>
              <Button className="bg-white text-[#205E98] hover:bg-white/90 font-bold rounded-2xl h-11 px-6">
                Find Doctors
              </Button>
            </Link>
            <Link href="/hospitals">
      ...
      <section className="container mx-auto max-w-5xl px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: <Stethoscope className="w-5 h-5 text-[#205E98]" />, label: "Verified Doctors", value: "50+", bg: "bg-blue-50" },
      ...
      <section className="container mx-auto max-w-5xl px-4 py-6">
        <h2 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-2">
          <Stethoscope className="w-5 h-5 text-[#205E98]" />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {HEALTHCARE_SPECIALTIES.slice(0, 8).map((specialty) => (
            <Link
              ...
              className="group bg-white border border-slate-100 rounded-2xl p-4 hover:border-[#205E98]/30 hover:shadow-md transition-all"
            >
              <p className="font-semibold text-slate-800 text-sm group-hover:text-[#205E98] transition-colors">
                {specialty}
              </p>
      ...
      <section className="container mx-auto max-w-5xl px-4 py-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link
            ...
            className="text-sm text-[#205E98] font-semibold flex items-center gap-1 hover:underline"
          >
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
          {districtDoctors.map((doctor) => (
            <Link
              ...
              className="group bg-white rounded-3xl border border-slate-100 p-4 hover:shadow-md hover:border-[#205E98]/20 transition-all"
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-slate-900 text-sm truncate group-hover:text-[#205E98] transition-colors">
                    {doctor.name}
                  </p>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-[#205E98]">{doctor.fee}</span>
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            {BIHAR_DISTRICTS.slice(0, 15).filter((d) => d !== districtFormatted).map((d) => (
              <Link
                ...
                className="text-xs font-medium bg-slate-50 border border-slate-200 text-slate-600 hover:bg-[#205E98]/8 hover:border-[#205E98]/30 hover:text-[#205E98] px-3 py-1.5 rounded-xl transition-all"
              >
                {d}
              </Link>
            ))}
